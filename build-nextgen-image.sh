...
case "$PROFILE" in
    ...
    6.18-ro)
        ...
        ;;
    6.18-bringup)
        # Factory/service bring-up image. It remains fully SD-booted until the
        # final NOR -> U-Boot -> NAND/UBI production chain is implemented.
...
if [ "$PROFILE" = "6.18-bringup" ]; then
    ...
    for sym in MTD_SPI_NOR MTD_SPI_NAND DRM DRM_FBDEV_EMULATION DRM_ATMEL_HLCDC VT VT_CONSOLE FRAMEBUFFER_CONSOLE FRAMEBUFFER_CONSOLE_DETECT_PRIMARY; do
    ...
    grep -q 'console=ttyS0,576000' "$NEXTGEN_UBOOT_ENV_TEXT" || {
        echo "error: bring-up environment lost the serial console" >&2
        exit 1
    }
    grep -q 'vt.global_cursor_default=0' "$NEXTGEN_UBOOT_ENV_TEXT" || {
        echo "error: bring-up environment does not suppress the VT cursor" >&2
        exit 1
    }
    grep -q 'nextgen.env=bringup' "$NEXTGEN_UBOOT_ENV_TEXT" || {
        echo "error: bring-up environment is missing nextgen.env=bringup" >&2
        exit 1
    }
    if grep -q 'console=tty0' "$NEXTGEN_UBOOT_ENV_TEXT"; then
        echo "error: bring-up environment must not make the LCD a kernel console" >&2
        exit 1
    fi
...
build_product()
{
    ...
    if [ "$PROFILE" = "6.18-bringup" ]; then
        ...
        grep -q 'post-build-bringup.sh' "$out/.config" || {
            echo "error: bring-up post-build hook is not configured" >&2
            exit 1
        }
    fi
    ...
    if [ "$PROFILE" = "6.18-bringup" ]; then
        for artifact in boot.vfat uboot.env rootfs.ext4 sdcard.img write-sd-card.sh nextgen-image-manifest.sha256; do
        ...
        (cd "$out/images" && sha256sum -c nextgen-image-manifest.sha256 >/dev/null) || {
            echo "error: bring-up image manifest verification failed" >&2
            exit 1
        }
