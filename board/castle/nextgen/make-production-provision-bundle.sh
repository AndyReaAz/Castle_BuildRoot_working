...
if [ "$#" -ne 7 ]; then
    echo "Usage: $0 <bundle-dir> <at91bootstrap.bin> <u-boot.bin> <u-boot.trailer> <boot.ubi> <rootfs.ubi> <product>" >&2
    exit 2
fi
...
for file in "$AT91" "$UBOOT" "$TRAILER" "$BOOT_UBI" "$ROOTFS_UBI"; do
    [ -f "$file" ] || {
        echo "error: missing production artifact: $file" >&2
        exit 1
    }
done
...
ROOTFS_UBI_BYTES="$(wc -c < "$ROOTFS_UBI" | tr -d '[:space:]')"
...
# Keep compatibility with the old bootstrap's fixed 0xA0000 read until the
# trailer-aware bootstrap is the final NOR object programmed.
[ "$UBOOT_BYTES" -le $((0x0a0000)) ] || {
    echo "error: production U-Boot exceeds 640 KiB migration window: $UBOOT_BYTES" >&2
    exit 1
}

[ "$(wc -c < "$TRAILER" | tr -d '[:space:]')" -eq 16 ] || {
    echo "error: U-Boot trailer is not exactly 16 bytes" >&2
    exit 1
}

set -- $(od -An -tu1 -N16 "$TRAILER")
[ "$1" -eq 78 ] && [ "$2" -eq 71 ] && [ "$3" -eq 85 ] && [ "$4" -eq 66 ] || {
    echo "error: U-Boot trailer has invalid NGUB magic" >&2
    exit 1
}
...
[ "$BOOT_UBI_BYTES" -le $((0x00800000)) ] || {
    echo "error: boot.ubi exceeds the 8 MiB image budget: $BOOT_UBI_BYTES" >&2
    exit 1
}
[ "$ROOTFS_UBI_BYTES" -le $((0x08000000)) ] || {
    echo "error: rootfs.ubi exceeds the 128 MiB partition: $ROOTFS_UBI_BYTES" >&2
    exit 1
}
...
install -m 0644 "$ROOTFS_UBI" "$OUT/rootfs.ubi"
