...
settings_valid()
{
    ...
    [ -s "$file" ] || return 1
    [ "$(settings_string "$file" FileFormat 2>/dev/null || true)" = "NextGenSettings" ] ||
        return 1
    [ "$(settings_uint "$file" SchemaVersion 2>/dev/null || true)" = "1" ] ||
        return 1
    [ "$(settings_string "$file" Product 2>/dev/null || true)" = "$PRODUCT" ] ||
        return 1
    ...
    settings_string "$file" UsbManufacturer >/dev/null || return 1
    settings_string "$file" UsbProduct >/dev/null || return 1
    return 0
}
...
read_gadget_identity()
{
    ...
    SETTINGS_INDEX="$(settings_count "$settings_file")"
