...
mkdir -p     "$PERSIST_SEED/app/$PRODUCT/active"     "$PERSIST_SEED/data"     "$PERSIST_SEED/state"     "$PERSIST_SEED/common-state"     "$PERSIST_SEED/os/NetworkManager/system-connections"     "$PERSIST_SEED/os/NetworkManager/state"     "$PERSIST_SEED/os/dbus"     "$PERSIST_SEED/os/chrony"     "$PERSIST_SEED/os/ssh/root"     "$PERSIST_SEED/os/seedrng"
chmod 0700 "$PERSIST_SEED/os/ssh/root"

# Root's home lives on the immutable system image. Keep engineering SSH
# authorized_keys writable/persistent without making /root writable.
rm -rf "$TARGET_DIR/root/.ssh"
ln -s /persist/os/ssh/root "$TARGET_DIR/root/.ssh"
...
rm -rf "$ROOT/app" "$ROOT/data" "$ROOT/state" "$ROOT/common/state" \
    "$TARGET_DIR/persist" "$TARGET_DIR/sdcard"
